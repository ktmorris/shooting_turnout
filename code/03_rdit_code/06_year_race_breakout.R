...
out <- rbindlist(lapply(c("b", "l", "w"), function(pl){
  rbindlist(lapply(c(seq(0.35, 1, 0.05), seq(2, 20, 1)), function(threshold){
    ...
    set_post <- set_post[!(paste0(set_post$GEOID, set_post$year) %in% paste0(set_pre$GEOID, set_pre$year)),
                         .SD[1], by = .(year, GEOID)] %>% 
      mutate(treated = F)
    rbindlist(lapply(c(2016, 2020), function(y){
      print(paste(y, pl, threshold))
      ...
      l <- try(rdrobust(y = full_treat$turnout, x = full_treat$d2, p = 1, c = 0, cluster = full_treat$id))
      
      l2 <- try(rdrobust(y = full_treat$turnout, x = full_treat$d2, p = 1, c = 0, cluster = full_treat$id,
      ...
      return(mutate(f, year = y))
    }))
  })) %>% 
    mutate(bw = pl)
}))

saveRDS(out, "temp/plu_data_years.rds")
out <- readRDS("temp/plu_data_years.rds")
...
out <- rbindlist(lapply(c("B", "H", "W"), function(r){
  rbindlist(lapply(c(seq(0.25, 1, 0.05)), function(threshold){
    ...
    rbindlist(lapply(c(2016, 2020), function(y){
      ...
      l <- try(rdrobust(y = full_treat$turnout, x = full_treat$d2, p = 1, c = 0, cluster = full_treat$id, h = 12))
      ...
      l2 <- try(rdrobust(y = full_treat$turnout, x = full_treat$d2, p = 1, c = 0, cluster = full_treat$id,
                         covs = select(full_treat,
                                       ...
                                       pop_dens, turnout_pre, some_college)))
      if(inherits(l2, "try-error")){
        l2 <- try(rdrobust(y = full_treat$turnout, x = full_treat$d2, p = 1, c = 0, cluster = full_treat$id, h = 10,
                           covs = select(full_treat,
                                         latino, nh_white, asian,
                                         nh_black, median_income, median_age,
                                         pop_dens, turnout_pre, some_college)))
...
saveRDS(out, "temp/race_data_years.rds")
out <- readRDS("temp/race_data_years.rds")
